...
public class PuzzleViewModel {
	...
	public void moveTile(Position pos) {
		...
		if (this.board.moveTile(pos, destinationPos)) {
			Move currMove = new Move(pos, destinationPos);
			this.allMoves.add(currMove);
			this.setTilesForView();
	...
	public void undo() {
		...
		Position previousPostion = this.allMoves.peek().getDestination();
		Position destinationPos = this.board.getEmptyTilePosition();
		Move undoMove = new Move(previousPostion, destinationPos);
		if (this.board.moveTile(undoMove)) {
			this.allMoves.pop();
			this.setTilesForView();
		}
	}
	 ...
	 */
	public void help() {
		Deque<Board> moveTiles = this.solver.bfsHelpAlogrithm(this.board);
