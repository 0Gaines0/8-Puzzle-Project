...
public class Board {
	 ...
	 */
	public Board(Board board, Board parent) {
		if (board == null) {
	...
	/**
	 * Gets the position from tile.
	 *
	 * @param tile the tile
	 * @return the position from tile
	 */
	public Position getPositionFromTile(int tile) {
		for (Position pos : Position.values()) {
			if (this.getTile(pos) == tile) {
				return pos;	
			}	
		}
		return null;
	...
	public boolean moveTile(Position src, Position dest) {
		...
		for (Position pos : this.getEmptyTilePosition().getNeighbors()) {
			if (pos.equals(src)) {
	...
	/**
	 * Gets the tiles for board in order
	 *
	 * @return the tiles
	 */
	private ArrayList<Integer> getTilesInOrder() {
		ArrayList<Integer> tiles = new ArrayList<Integer>();
		for (Position currPos : Position.values()) {
			tiles.add(this.getTile(currPos));
		}
		return tiles;
	}
	
	/**
	 * Checks if board is solveable.
	 *
	 * @return true, if is solveable
	 */
	public boolean isSolveable() {
		return this.getInversionCount(this.getTilesInOrder()) % 2 == 0;
	}
	 ...
	 * @return the list
	 */
	public List<Board> neighborsBoards() {
		List<Board> neighborBoards = new ArrayList<Board>();
		for (Position newPos : this.getEmptyTilePosition().getNeighbors()) {
			Board neighbor = new Board(this);
			Move currMove = new Move(this.getEmptyTilePosition(), newPos);
			Move blankMove = new Move(newPos, this.getEmptyTilePosition());
			neighbor.moveTile(currMove);
			neighbor.moveTile(blankMove);
	...
	public String toString() {
		ArrayList<Integer> tilesInOrder = new ArrayList<Integer>();
		for (Position currPos : Position.values()) {
			tilesInOrder.add(this.getTile(currPos));
		}
		return "Board [tiles=" + tilesInOrder + "]";
	}
